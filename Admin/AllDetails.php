                     ...
                     <div class="row">
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                            <div class="page-header">
                               <h1 class="pageheader-title" style="text-align:center;font-size:35px;background-color: #f8d7da;border: 5px solid transparent;border-radius: .25rem;color: MediumSeaGreen;font-family: 'Circular Std Book';line-height: 40px;;font-weight: bold;" >صفحة كافة التفاصيل لكلية معينة</h2>
...
$count=$row["c"];
			?>
					...
					<!--  -->
					   <div class="card" dir=rtl>
                            <h1 class="card-header" style="text-align:center;color:red;font-style: normal;font-family: 'Circular Std Book';font-weight: bold;">ندوات كلية <?php echo $faculty1;?> العدد الاجمالي :<?php echo $count;?></h2>
                              <div class="table-responsive" dir=rtl>       
                                    <table id="dataTables-example1" class="table table-striped table-bordered table-hover"  dir=rtl>
...
$sql = "SELECT count(*) as c FROM courses where id_user='$id_user'";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_array($result);
$count=$row["c"];
			?>
					<!----------------------------------------------------------------------------------------->
					   <div class="card" dir=rtl>
                            <h1 class="card-header" style="text-align:center;color:red;font-style: normal;font-family: 'Circular Std Book';font-weight: bold;">الدورات التدريبية لكلية <?php echo $faculty1;?> العدد الاجمالي :<?php echo $count;?></h1>
                              <div class="table-responsive" dir=rtl>       
                                    <table id="dataTables-example2" class="table table-striped table-bordered table-hover"  dir=rtl>
                                    <thead style="background-color: DodgerBlue;font-size:17px; ">
                                        <tr>
											<th style="text-align: center;">الكلية</th>
										<th style="text-align: center;">عنوان الدورة
</th>
<th style="text-align: center;">مكان الانعقاد</th>
<th style="text-align: center;">التاريخ</th>
<th style="text-align: center;">صور التوثيق</th>
									...
									<?php
									include_once("phpcode/dbconec.php");
$sql = "SELECT courses.`id` as sid,courses.`id_user`,courses.title,courses.lectureres,courses.`Target_group`,courses.`seminar_Place`,courses.date,courses.number_participates,courses.participating_party,courses.seminar_objectives,courses.connection_with,courses.Images_Committee,user.faculty FROM courses,user where user.id=courses.id_user and courses.id_user='$id_user' order by courses.id desc";
$result = mysqli_query($conn,$sql);
if($result){
										...
										$Images_Committee = $row['Images_Committee'];
											...
											if($id % 2 ==0 )
											{
												...
												<td style='text-align: center;'>".$seminar_Place."</td>
													...
													";
													  if($Images_Committee!=-1)
													      echo "<td style='text-align: center;'><a href='downloadCourse.php?file_id=".$id ."&file_name=".$Images_Committee."'>تحميل</a></td>";
														  else
														  echo "<td style='text-align: center;color:red;'>'لأ يوجد'</td>";														  
													echo "<td style='text-align: center;'><a href=showscoursedetails.php?eid=".$id ." <button class='btn btn-primary'> <i class='fa fa-edit' ></i> التفاصيل</button></td>
							...
							<?php 
$sql = "SELECT count(*) as c FROM job where id_user='$id_user'";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_array($result);
$count=$row["c"];
			?>
					<!-------------------------------------------------------------------------------------------->
					   <div class="card" dir=rtl>
                             <h1 class="card-header" style="text-align:center;color:red;font-style: normal;font-family: 'Circular Std Book';font-weight: bold;">ورش العمل لكلية <?php echo $faculty1;?> العدد الاجمالي :<?php echo $count;?></h1>
                              <div class="table-responsive" dir=rtl>       
                                   <table id="dataTables-example3" class="table table-striped table-bordered table-hover"  dir=rtl>
                                    <thead style="background-color: DodgerBlue;font-size:17px; ">
                                        <tr>
											<th style="text-align: center;">الكلية</th>
										<th style="text-align: center;">عنوان الورشة
</th>
<th style="text-align: center;">مكان الانعقاد</th>
<th style="text-align: center;">التاريخ</th>
<th style="text-align: center;">صور التوثيق</th>
									...
									<?php
									include_once("phpcode/dbconec.php");
$sql = "SELECT job.`id` as sid,job.`id_user`,job.title,job.lectureres,job.`Target_group`,job.`seminar_Place`,job.date,job.number_participates,job.participating_party,job.seminar_objectives,job.connection_with,job.Images_Committee,user.faculty FROM job,user where user.id=job.id_user and job.id_user='$id_user' order by job.id desc";
$result = mysqli_query($conn,$sql);
if($result){
										...
										$Images_Committee = $row['Images_Committee'];
											...
											if($id % 2 ==0 )
											{
												...
												<td style='text-align: center;'>".$seminar_Place."</td>
													...
													";
													  if($Images_Committee!=-1)
													      echo "<td style='text-align: center;'><a href='downloadJob.php?file_id=".$id ."&file_name=".$Images_Committee."'>تحميل</a></td>";
														  else
														  echo "<td style='text-align: center;color:red;'>'لأ يوجد'</td>";														  
													echo "<td style='text-align: center;'><a href=showsjobdetails.php?eid=".$id ." <button class='btn btn-primary'> <i class='fa fa-edit' ></i> التفاصيل</button></td>
									...
									?>
												...
												<?php 
$sql = "SELECT count(*) as c FROM Conferences where id_user='$id_user'";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_array($result);
$count=$row["c"];
			?>
					<!-------------------------------------------------------->
					   <div class="card" dir=rtl>
                             <h1 class="card-header" style="text-align:center;color:red;font-style: normal;font-family: 'Circular Std Book';font-weight: bold;">المؤتمرات والمنتديات لكلية <?php echo $faculty1;?> العدد الاجمالي :<?php echo $count;?></h1>
                              <div class="table-responsive" dir=rtl>       
                                     <table id="dataTables-example4" class="table table-striped table-bordered table-hover"  dir=rtl>
                                    <thead style="background-color: DodgerBlue;font-size:17px; ">
                                        <tr>
											<th style="text-align: center;">الكلية</th>
										<th style="text-align: center;">عنوان المؤتمر
</th>
<th style="text-align: center;">رعاة المؤتمر
</th>
<th style="text-align: center;">مكان الانعقاد</th>
<th style="text-align: center;">التاريخ</th>
<th style="text-align: center;">صور التوثيق</th>
...
if($result){
					...
					<?php 
$sql = "SELECT count(*) as c FROM others where id_user='$id_user'";
$result = mysqli_query($conn,$sql);
$row = mysqli_fetch_array($result);
$count=$row["c"];
			?>
					<!-------------------------------------------------------->
					   <div class="card" dir=rtl>
                             <h1 class="card-header" style="text-align:center;color:red;font-style: normal;font-family: 'Circular Std Book';font-weight: bold;">الفعاليات الأخرى (قوافل – احتفالية- غيرها) لكلية <?php echo $faculty1;?> العدد الاجمالي :<?php echo $count;?></h1>
                              <div class="table-responsive" dir=rtl>       
                                      <table id="dataTables-example" class="table table-striped table-bordered table-hover"  dir=rtl>
                                    <thead style="background-color: DodgerBlue;font-size:17px; ">
                                        <tr>
											<th style="text-align: center;">الكلية</th>
										<th style="text-align: center;">عنوان الفعالية
</th>
<th style="text-align: center;">راعى الفاعلية </th>
<th style="text-align: center;">التاريخ</th>
<th style="text-align: center;">صور التوثيق</th>
 ...
 <th style="text-align: center;">التفاصيل</th>
									...
									$faculty=$_SESSION['faculty'];
									include_once("phpcode/dbconec.php");
$sql = "SELECT others.`id` as sid,others.`id_user`,others.title,others.sponsors,others.`Target_group`,others.`seminar_Place`,others.date,others.number_participates,others.participating_party,others.seminar_objectives,others.connection_with,others.Images_Committee,user.faculty FROM others,user where user.id=others.id_user and others.id_user='$id_user' order by others.id desc";
$result = mysqli_query($conn,$sql);
if($result){
										...
										$Images_Committee = $row['Images_Committee'];
											...
											if($id % 2 ==0 )
											{
												...
												<td style='text-align: center;'>".$faculty."</td>
												<td style='text-align: center;'>".$title."</td>
												<td style='text-align: center;'>".$sponsors."</td>
													<td style='text-align: center;'>".$date."</td>
													
													";
													  if($Images_Committee!=-1)
													      echo "<td style='text-align: center;'><a href='downloadOthers.php?file_id=".$id ."&file_name=".$Images_Committee."'>تحميل</a></td>";
														  else
														  echo "<td style='text-align: center;color:red;'>'لأ يوجد'</td>";														  
													echo "<td style='text-align: center;'><a href=showsothersdetails.php?eid=".$id ." <button class='btn btn-primary'> <i class='fa fa-edit' ></i> التفاصيل</button></td>
