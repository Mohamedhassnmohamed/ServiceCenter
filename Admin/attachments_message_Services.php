...
$meesage_id = $_GET['meesage_id'];
			$sql = "SELECT *  FROM check_form,services where check_form.id=services.id_message and  services.id_message='$meesage_id'";
...
$reserach_type=$row["reserach_type"];
 ...
 <h2 class="card-header" class="text-right"  style="text-align:center;color:red;font-style: normal;font-family: 'Circular Std Book';font-weight: bold;">درجة الرسالة :  <?php echo $reserach_type;?></h3>
			...
			$sql = "SELECT *  FROM message_attachments_services where message_id='$meesage_id' and services_id='$services_id'";
$result = mysqli_query($conn,$sql);
if($result){
		...
		<td style='text-align: center;'><a href='downloadattach_Services.php?file_id=".$id ."&file_name=".$file_path."'><i class='fa fa-download' style='font-size:25px;color:red'></a></td>
													<td style='text-align: center;'><a href=phpcode/deleteattach_Services.php?eid=".$id ." <button class='btn btn-danger' > <i class='fa fa-edit' ></i> حذف</button></td>
    ...
    <div class="modal-content">
      <div class="modal-header" dir=rtl style="text-align">
       ...
       <div class="modal-body">
                                <form enctype="multipart/form-data" method="post" action="phpcode/adddmessageattachments_Services.php" autocomplete="off">
          <div class="form-group">
            <label for="message-text" class="col-form-label" style="text-align: right;font-size:22px;">عنوان الرسالة</label>
            <input type="text" class="form-control"  name="name"  value="<?php echo $message_title;?>"/>
          </div>
		  <div class="form-group">
...
<option value="طلب الحصول علي خدمة">طلب الحصول علي خدمة</option>
<option value="نموذج طلب خدمة البحث">نموذج طلب خدمة البحث</option>
<option value="إيصال سداد خدمة">إيصال سداد خدمة</option>
																											
<option value="نموذج طلب خدمات المراجعة">نموذج طلب خدمات المراجعة</option>
<option value="إفادة بخدمات المراجعة">إفادة بخدمات المراجعة</option>
<option value="نموذج طلب تحليل إحصائي">نموذج طلب تحليل إحصائي</option>
<option value="إيصال دفع نموذج  البحث عن المصادر">إيصال دفع نموذج  البحث عن المصادر</option>
			<option value="إيصال دفع طلب تحليل إحصائي">إيصال دفع طلب تحليل إحصائي</option>																																																																																																																		<option value="نموذج طلب تحليل إحصائي">نموذج طلب تحليل إحصائي</option>
...
<input type="file" class="form-control"  name="file_name"/>
 ...
 <input type="hidden" name="message_id" value="<?php echo $meesage_id ;?>"/>
